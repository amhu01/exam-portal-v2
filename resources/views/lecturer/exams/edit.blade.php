                    ...
                    <form method="POST" action="{{ route('lecturer.exams.update', $exam) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" rows="3"
                                class="w-full border rounded px-3 py-2">{{ old('description', $exam->description) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label class="block text-gray-700 font-bold mb-2">Class & Subject</label>
                            <select name="class_subject_id" class="w-full border rounded px-3 py-2">
                                @foreach($classSubjects as $classSubject)
                                    <option value="{{ $classSubject->id }}" {{ $exam->class_subject_id == $classSubject->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="time_limit" value="{{ old('time_limit', $exam->time_limit) }}" min="1" max="180"
